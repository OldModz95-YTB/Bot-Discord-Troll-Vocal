...
const Commands = {
	'target': {
		help: 'Set the person that Troll will target. Usage: **target** @oldmodz95 . Must @ (mention) a valid user.',
	...
	'help': {
		help: 'List commands for Troll.',
		execute: (message) => {
			let helpMessage = new Discord.MessageEmbed()
			.setTitle('Troll Bot Help');
...
Client.on('ready', () => {
	console.log('BOT TROLL IN STARTING ! SEND VICTIM !!!!!!');
});
...
Client.on('message', (message) => {
	...
	if (content.startsWith(prefix)) {
		...
		if (Commands[cmd]) {
			...
			message.reply('Command not found, use "help" to see commands.');
		}
	}
});
...
// check to see if Troll is already talking and if not make Troll speak
// when member 
...
// This function plays the Troll audio 
// it will recursively play while the target
// is still speaking.
const play = (connection) => {
	dispatcher = connection.play('./ntm.mp3')
	.on('finish', () => {
		if (isTalking) {
			play(connection)
		}
